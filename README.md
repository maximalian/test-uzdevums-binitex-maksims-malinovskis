...
| Валидация/UX                                          | Сделано | Даты ограничены min/max API; числовые поля ожидают числа (без отдельного UI ошибок).                                |
...
| Validācija/UX                                          | Izpildīts | Datumi ierobežoti ar min/max no API; skaitliskie lauki sagaida skaitļus (bez atsevišķa kļūdu UI).              |
